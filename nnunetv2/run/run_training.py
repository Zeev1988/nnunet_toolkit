...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
    ...
    nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, fold=fold,
                                    dataset_json=dataset_json, epochs=epochs, device=device,
...
def run_training(dataset_name_or_id: Union[str, int],
    ...
    if num_gpus > 1:
        ...
        nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, epochs, trainer_class_name,
                                               plans_identifier, device=device, log_container=log_container)
...
def main(params=None):
    """
    Simplified main function that uses nnU-Net v2's run_training function directly
    """
    parser = argparse.ArgumentParser()
    parser.add_argument("-c", "--configuration", default='3d_fullres')
    parser.add_argument("-tr", "--trainer", default='nnUNetTrainer')
    parser.add_argument("-d", "--dataset", default='Dataset004_Hippocampus', help="Dataset name")
    parser.add_argument("-f", "--fold", default='0', help='0, 1, ..., 4 or \'all\'')
    parser.add_argument("--val", "--validation_only", help="use this if you want to only run the validation",
                        action="store_true")
    parser.add_argument("--continue_training", help="continue a previous training",
                        action="store_true")
    parser.add_argument("-p", "--plans", help="plans identifier",
                        default="nnUNetPlans", required=False)
    parser.add_argument("--npz", required=False, default=False, action="store_true",
                        help="save softmax predictions as npz files")
    parser.add_argument("--val_best", required=False, default=False, action="store_true",
                        help="use best checkpoint for validation")
    parser.add_argument("-device", default='cuda', help="Use this to set the device the training should run with")

    args = parser.parse_args()

    # Extract parameters from GUI or use args
    if params is not None:
        ...
        log_container = params["log_container"]
        # Convert dataset name/id to proper format
        if isinstance(dataset_name, int):
            dataset_name = f"Dataset{dataset_name:03d}"
        dataset_name = maybe_convert_to_dataset_name(dataset_name)
    else:
        fold = args.fold
    ...
    # ===== DETERMINE TRAINER CLASS BASED ON LOSS FUNCTION =====
    trainer_class_name = 'nnUNetTrainer'

    if loss_function in ("RobustCrossEntropyLoss", "Robust Cross-Entropy Loss"):
        trainer_class_name = 'nnUNetTrainerCELoss'
    elif loss_function in ("TopK10"):
        trainer_class_name = 'nnUNetTrainerTopk10Loss'
    elif loss_function in ("Dice_Focal", "Dice", "Dice Focal"):
        trainer_class_name = 'nnUNetTrainerDiceLoss'
    ...
    return None  # v2 doesn't return trainer object
