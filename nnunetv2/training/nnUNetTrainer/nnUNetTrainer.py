...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, epochs: int = 1000,
                 device: torch.device = torch.device('cuda'), log_container=None):
        ...
        for k in inspect.signature(self.__init__).parameters.keys():
            if k != 'log_container':  # Exclude Streamlit container
    ...
    def print_to_log_file(self, *args, also_print_to_console=True, add_timestamp=True):
        if self.local_rank == 0:
            ...
            # Update Streamlit widget if provided
            if self.log_container is not None:
                self.update_streamlit_logs(log_message)
    ...
    def update_streamlit_logs(self, new_log_message):
        """Update the Streamlit logs display"""
        ...
        self.log_container.text_area("Logs:", value=all_logs, height=400, disabled=True)
